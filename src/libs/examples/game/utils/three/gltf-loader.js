...
const GLTFLoader = (function () {
    ...
    parse: function (data, path, onLoad, onError) {
      ...
      if (json.extensionsUsed) {
        ...
        if (json.extensionsUsed.indexOf(EXTENSIONS.KHR_MATERIALS_UNLIT) >= 0) {
          extensions[EXTENSIONS.KHR_MATERIALS_UNLIT] =
        ...
        if (
          json.extensionsUsed.indexOf(EXTENSIONS.KHR_DRACO_MESH_COMPRESSION) >=
          0
        ) {
  ...
  GLTFMaterialsUnlitExtension.prototype.getMaterialType = (material) =>
  ...
  const GLTFMaterialsPbrSpecularGlossinessExtension = () => ({
    ...
    getMaterialType: () => THREE.ShaderMaterial,
  ...
  GLTFParser.prototype.getMultiDependencies = function (types) {
    ...
    return Promise.all(pendings).then(() => results);
  };
        ...
        return scope.loadGeometries(primitives).then(function (geometries) {
          for (var i = 0, il = primitives.length; i < il; i++) {
            ...
            if (useMorphTargets) {
              ...
              if (mesh.geometry.morphAttributes.normal !== undefined)
