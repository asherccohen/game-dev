...
export default async ({ parent, x = 0, y = 0, z = 0 }) => {
  const gltf = useLoader(GLTFLoader, "../../assets/models/jet.glb");
  ...
  timelines.controls = {
    ...
    update(self, entities, { directions }, { gamepadController }) {
      ...
      if (gamepadController.leftStick.heading !== null) {
        const degrees = THREE.Math.radToDeg(
          gamepadController.leftStick.heading
        );
        const direction = directions.find((x) =>
          between(degrees, x.heading - 30, x.heading + 30)
        );
